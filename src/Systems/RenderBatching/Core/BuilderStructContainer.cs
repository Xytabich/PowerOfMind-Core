...
{
	public class BuilderStructContainer<TVertex, TUniform> : IBuilderStructContainer
		...
		where TUniform : unmanaged, IUniformsData
	{
		...
		unsafe void IBuilderStructContainer.ProvideVertexData(RawDataProcessor processor)
		{
		...
		unsafe void IBuilderStructContainer.DiffUniforms<T>(in T otherData, Dictionary<int, int> uniformsMap,
			Dictionary<int, int> otherMap, List<KeyValuePair<int, int>> tmpList, UniformsDataCollection uniformsData)
		{
			tmpList.Clear();
			...
			fixed(T* oPtr = &otherData)
			{
				fixed(TUniform* uPtr = &uniformDefaults)
				{
					foreach(var pair in tmpList)
					{
						...
						if(MemUtils.MemEquals((byte*)uPtr + uniforms[index].Offset, (byte*)oPtr + otherUniforms[index].Offset, size))
						{
							otherMap.Remove(pair.Key);
						}
						else
	...
	public interface IBuilderStructContainer
	{
		void Init();
		...
		void ProvideVertexData(RawDataProcessor processor);
	...
	public unsafe delegate void RawDataProcessor(byte* ptr);
}