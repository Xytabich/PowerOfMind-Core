namespace PowerOfMind.Graphics.Shader
{
	public static partial class ShaderParser
	{
		private static class ShaderParserHelpers
		{
			...
			public static TokenSourceRef? CollectVersion(string source, List<Token> tokens, TokenSourceRef start, ref int versionToken, params TryProcessTokensDelegate[] subProcessors)
			{
				var token = tokens[start.index];
				if(token.type == TokenType.Hash && token.size - start.offset == 1)
				{
					var from = start;
					start = start.Step(tokens);
					if(IsLetter(source, tokens, start, "version"))
					{
						start = start.Step(tokens);
						if(ProcessIntervalUntil(tokens, ref start, p => tokens[p.index].type == TokenType.Number, subProcessors))
						{
							versionToken = start.index;
			...
			public static TokenSourceRef? CollectUniform(string source, List<Token> tokens, TokenSourceRef start, List<FieldInfo> outList, ICollection<TokenRange> outSkipList, params TryProcessTokensDelegate[] subProcessors)
			{
				//[MVP_MATRIX]
				//uniform mat4 mvp;
				...
				if(hasAlias)
				{
					...
					{
						outSkipList.Add(new TokenRange(new TokenRef(start.index, start.offset), new TokenRef(position.index, position.offset), position.sourceOffset - start.sourceOffset));
					}
					else
					{
						outSkipList.Add(new TokenRange(new TokenRef(start.index, start.offset), new TokenRef(position.index - 1, 0), position.sourceOffset - start.sourceOffset));
					}
				}
				else position = start;
			...
			public static TokenSourceRef? CollectInput(string source, List<Token> tokens, TokenSourceRef start, List<FieldInfo> outList, ICollection<TokenRange> outSkipList, params TryProcessTokensDelegate[] subProcessors)
			{
				//[POSITION]
